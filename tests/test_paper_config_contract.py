...
def test_release_metadata_is_fixed_at_0_1_1():
    pyproject = (PUBLIC_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    citation = (PUBLIC_ROOT / "CITATION.cff").read_text(encoding="utf-8")

    assert re.search(r'^version = "0\.1\.1"$', pyproject, flags=re.MULTILINE)
    assert re.search(r"^version: 0\.1\.1$", citation, flags=re.MULTILINE)
    assert re.search(r"^date-released: 2026-08-14$", citation, flags=re.MULTILINE)
    assert __import__("cordiag").__version__ == "0.1.1"
